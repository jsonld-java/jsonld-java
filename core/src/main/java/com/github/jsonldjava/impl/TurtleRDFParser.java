...
import static com.github.jsonldjava.core.Regex.*;
import static com.github.jsonldjava.core.RDFDatasetUtils.unescape;

/**
 * A (probably terribly slow) Parser for turtle -> the internal RDFDataset used by JSOND-Java
 *
...
public class TurtleRDFParser implements RDFParser {
	...
	static class Regex {
		final public static Pattern PREFIX_ID = Pattern.compile("@prefix" + WS_1_N + PNAME_NS + WS_1_N + IRIREF + WS_0_N + "\\." + WS_0_N);
		final public static Pattern BASE = Pattern.compile("@base" + WS_1_N + IRIREF + WS_0_N + "\\." + WS_0_N);
		final public static Pattern SPARQL_PREFIX = Pattern.compile("[Pp][Rr][Ee][Ff][Ii][Xx]" + WS + PNAME_NS + WS + IRIREF + WS_0_N);
		final public static Pattern SPARQL_BASE = Pattern.compile("[Bb][Aa][Ss][Ee]" + WS + IRIREF + WS_0_N);
		
		final public static Pattern PREFIXED_NAME = Pattern.compile("(?:" + PNAME_LN + "|" + PNAME_NS + ")");
		...
		final public static Pattern STRING = Pattern.compile("(" + STRING_LITERAL_LONG_SINGLE_QUOTE + "|" + STRING_LITERAL_LONG_QUOTE + "|" + STRING_LITERAL_QUOTE + "|" + STRING_LITERAL_SINGLE_QUOTE + ")");
		final public static Pattern BOOLEAN_LITERAL = Pattern.compile("(true|false)");
		final public static Pattern RDF_LITERAL = Pattern.compile(STRING + "(?:" + LANGTAG + "|\\^\\^" + IRI + ")?");
		final public static Pattern NUMERIC_LITERAL = Pattern.compile("(" + DOUBLE + ")|(" + DECIMAL + ")|(" + INTEGER + ")");
		final public static Pattern LITERAL = Pattern.compile(RDF_LITERAL + "|" + NUMERIC_LITERAL + "|" + BOOLEAN_LITERAL);

		final public static Pattern DIRECTIVE = Pattern.compile("^(?:" + PREFIX_ID + "|" + BASE + "|" + SPARQL_PREFIX + "|" + SPARQL_BASE + ")");
		...
		final public static Pattern OBJECT = Pattern.compile("^" + IRI + "|" + BLANK_NODE + "|" + LITERAL);

		// others
		//final public static Pattern WS_AT_LINE_START = Pattern.compile("^" + WS_1_N);
		...
		final public static Pattern COMMENT_OR_WS = Pattern.compile("^(?:(?:[#].*(?:" + EOLN + ")" + WS_0_N + ")|(?:" + WS_1_N + "))");
	}
	...
	private class State {
		...
		//int bnodes = 0;
		UniqueNamer namer = new UniqueNamer("_:b");// {{ getName(); }}; // call getName() after construction to make first active bnode _:b1
		...
		private void advanceLineNumber() throws JSONLDProcessingError {
			Matcher match = Regex.NEXT_EOLN.matcher(line);
			if (match.find()) {
		...
		public void advanceLinePosition(int len) throws JSONLDProcessingError {
			...
			while (!"".equals(line)) {
				...
				Matcher match = Regex.COMMENT_OR_WS.matcher(line);
		...
		while (!"".equals(state.line)) {
			...
			if (state.curSubject == null) {
				...
				// handle collections
					...
					String bnode = state.namer.getName();
					// so we know we want a predicate if the collection close isn't followed by a subject end 
					state.curSubject = bnode;
					state.advanceLinePosition(1);
					state.push();
					state.curSubject = bnode;
	...
	final public static Pattern IRIREF_MINUS_CONTAINER = Pattern.compile("(?:(?:[^\\x00-\\x20<>\"{}|\\^`\\\\]|" + UCHAR + ")*)|" + Regex.PREFIXED_NAME);
	...
	private void validateIRI(State state, String iri) throws JSONLDProcessingError {
		if (!IRIREF_MINUS_CONTAINER.matcher(iri).matches()) {
			throw new JSONLDProcessingError("Error while parsing Turtle; invalid IRI after escaping.")
				.setType(JSONLDProcessingError.Error.PARSE_ERROR)
				.setDetail("line", state.lineNumber)
				.setDetail("position", state.linePosition);
		}
	...
	final private static Pattern PN_LOCAL_ESC_MATCHED = Pattern.compile("[\\\\]([_~\\.\\-!$&'\\(\\)*+,;=/?#@%])");

	static String unescapeReserved(String str) {
		if (str != null) {
			Matcher m = PN_LOCAL_ESC_MATCHED.matcher(str);
			if (m.find()) {
