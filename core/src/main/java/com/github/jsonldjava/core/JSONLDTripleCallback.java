...
public interface JSONLDTripleCallback {
	 ...
	 * 
	 * @param dataset
	 * The format of the dataset is a Map with the following structure:
	 * { 
	 ...
	 * }
	 * 
	 * GRAPH: Is the graph name/IRI. if no graph is present for a triple, it will be listed under the "@default" graph
	 * TRIPLE: Is a map with the following structure:
	 * {
	 * 		"subject" : SUBJECT
	 * 		"predicate" : PREDICATE
	 * 		"object" : OBJECT
	 * }
	 * 
	 * Each of the values in the triple map are also maps with the following key-value pairs:
