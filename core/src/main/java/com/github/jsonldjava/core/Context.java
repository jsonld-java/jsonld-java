...
public class Context extends LinkedHashMap<String, Object> {
	...
	public Object compactValue(String activeProperty, Map<String, Object> value) {
		...
		if (value.containsKey("@index")
				&& "@index".equals(this.getContainer(activeProperty))) {
			numberMembers--;
		}
		// 3)
		if (numberMembers > 2) {
		...
		if (value.containsKey("@type")
				&& Obj.equals(value.get("@type"), typeMapping)) {
			return valueValue;
		}
		// 6)
		...
		// 7)
		if (numberMembers == 1
				&& (!(valueValue instanceof String)
						|| !this.containsKey("@language") || (getTermDefinition(
	 ...
	 */
	public Context parse(Object localContext, List<String> remoteContexts)
			throws JsonLdError {
		...
		for (Object context : ((List<Object>) localContext)) {
			...
			// 3.2)
				...
				if (remoteContexts.contains(uri)) {
					throw new JsonLdError(Error.RECURSIVE_CONTEXT_INCLUSION,
							uri);
				}
			...
			if (remoteContexts.isEmpty()
					&& ((Map<String, Object>) context).containsKey("@base")) {
				final Object value = ((Map<String, Object>) context)
						.get("@base");
				if (value == null) {
					result.remove("@base");
					...
					if (JsonLdUtils.isAbsoluteIri((String) value)) {
						...
						if (!JsonLdUtils.isAbsoluteIri(baseUri)) {
							throw new JsonLdError(Error.INVALID_BASE_IRI,
									baseUri);
						}
						result.put("@base",
								JsonLdUrl.resolve(baseUri, (String) value));
					}
				} else {
			...
			for (final String key : ((Map<String, Object>) context).keySet()) {
				if ("@base".equals(key) || "@vocab".equals(key)
						|| "@language".equals(key)) {
					continue;
				}
				result.createTermDefinition((Map<String, Object>) context, key,
						defined);
			}
		}
		return result;
	...
	private void createTermDefinition(Map<String, Object> context, String term,
		...
		if (val.containsKey("@type")) {
			if (!(val.get("@type") instanceof String)) {
				throw new JsonLdError(Error.INVALID_TYPE_MAPPING,
						val.get("@type"));
			}
			String type = (String) val.get("@type");
			try {
				type = this.expandIri((String) val.get("@type"), false, true,
						context, defined);
		...
		if (val.containsKey("@reverse")) {
			...
			if (!(val.get("@reverse") instanceof String)) {
				throw new JsonLdError(Error.INVALID_IRI_MAPPING,
						"Expected String for @reverse value. got "
								+ (val.get("@reverse") == null ? "null" : val
										.get("@reverse").getClass()));
			}
			final String reverse = this.expandIri((String) val.get("@reverse"),
					false, true, context, defined);
			...
			if (val.containsKey("@container")) {
				...
				if (container == null || "@set".equals(container)
		...
		if (val.get("@id") != null && !term.equals(val.get("@id"))) {
			...
			final String res = this.expandIri((String) val.get("@id"), false,
					true, context, defined);
			if (JsonLdUtils.isKeyword(res) || JsonLdUtils.isAbsoluteIri(res)) {
				if ("@context".equals(res)) {
					throw new JsonLdError(Error.INVALID_KEYWORD_ALIAS,
		...
		// 14)
			...
			if (termDefinitions.containsKey(prefix)) {
				definition.put(
						"@id",
						((Map<String, Object>) termDefinitions.get(prefix))
								.get("@id") + suffix);
			} else {
		...
		if (val.containsKey("@language") && !val.containsKey("@type")) {
			if (val.get("@language") == null
					|| val.get("@language") instanceof String) {
				final String language = (String) val.get("@language");
				definition.put("@language",
	...
	String expandIri(String value, boolean relative, boolean vocab,
			Map<String, Object> context, Map<String, Boolean> defined)
			throws JsonLdError {
		// 1)
		...
		if (colIndex >= 0) {
			...
			if (this.termDefinitions.containsKey(prefix)) {
				return (String) ((LinkedHashMap<String, Object>) this.termDefinitions
						.get(prefix)).get("@id") + suffix;
			}
			// 4.5)
			return value;
		}
	 ...
	 */
	String compactIri(String iri, Object value, boolean relativeToVocab,
			boolean reverse) {
		// 1)
		if (iri == null) {
			return null;
		...
		if (relativeToVocab && getInverse().containsKey(iri)) {
			...
			// 2.6)
					...
					&& ((Map<String, Object>) value).containsKey("@list")) {
				// 2.6.1)
				...
				final List<Object> list = (List<Object>) ((Map<String, Object>) value)
						.get("@list");
				// 2.6.3)
				String commonLanguage = (list.size() == 0) ? defaultLanguage
						: null;
				...
				for (final Object item : list) {
					...
					if (JsonLdUtils.isValue(item)) {
						// 2.6.4.2.1)
						if (((Map<String, Object>) item)
								.containsKey("@language")) {
							itemLanguage = (String) ((Map<String, Object>) item)
									.get("@language");
						}
						// 2.6.4.2.2)
						else if (((Map<String, Object>) item)
								.containsKey("@type")) {
							itemType = (String) ((Map<String, Object>) item)
									.get("@type");
						}
						// 2.6.4.2.3)
						else {
					...
					// 2.6.4.5)
					else if (!commonLanguage.equals(itemLanguage)
							&& JsonLdUtils.isValue(item)) {
						commonLanguage = "@none";
					}
					// 2.6.4.6)
					if (commonType == null) {
					...
					// 2.6.4.8)
					if ("@none".equals(commonLanguage)
							&& "@none".equals(commonType)) {
						break;
					}
				}
				// 2.6.5)
				commonLanguage = (commonLanguage != null) ? commonLanguage
						: "@none";
				// 2.6.6)
			...
			// 2.12)
			if (("@reverse".equals(typeLanguageValue) || "@id"
					.equals(typeLanguageValue))
					&& (value instanceof Map)
					&& ((Map<String, Object>) value).containsKey("@id")) {
				// 2.12.1)
				final String result = this.compactIri(
						(String) ((Map<String, Object>) value).get("@id"),
						null, true, true);
			...
			// 2.14)
			final String term = selectTerm(iri, containers, typeLanguage,
					preferredValues);
			// 2.15)
			if (term != null) {
				return term;
		...
		for (final String term : termDefinitions.keySet()) {
			...
			final String candidate = term
					+ ":"
					+ iri.substring(((String) termDefinition.get("@id"))
							.length());
			// 5.4)
			if ((compactIRI == null || compareShortestLeast(candidate,
					compactIRI) < 0)
					&& (!termDefinitions.containsKey(candidate) || (iri
							.equals(((Map<String, Object>) termDefinitions
									.get(candidate)).get("@id")) && value == null))) {
	 ...
	 * <p>
	 * No guarantees of the prefixes are given, 
	 * beyond that it will not contain ":".
	 * 
	 * @param onlyCommonPrefixes
	 *            If <code>true</code>, the result will not include
	 *            "not so useful" prefixes, such as "term1":
	 *            "http://example.com/term1", e.g. all IRIs will 
	 *            end with "/" or "#". If <code>false</code>, all
	...
	public Map<String, String> getPrefixes(boolean onlyCommonPrefixes) {
		...
		for (final String term : termDefinitions.keySet()) {
			...
			Map<String, Object> termDefinition = (Map<String, Object>) termDefinitions
					.get(term);
	...
	public Context clone() {
		...
		// the tests!
		rval.termDefinitions = new LinkedHashMap<String, Object>(
	...
	public Map<String, Object> getInverse() {
		...
		for (final String term : terms) {
			final Map<String, Object> definition = (Map<String, Object>) termDefinitions
					.get(term);
			// 3.1)
			if (definition == null) {
				continue;
			...
			// 3.4 + 3.5)
			Map<String, Object> containerMap = (Map<String, Object>) inverse
					.get(iri);
			...
			// 3.6 + 3.7)
			Map<String, Object> typeLanguageMap = (Map<String, Object>) containerMap
					.get(container);
			if (typeLanguageMap == null) {
				typeLanguageMap = new LinkedHashMap<String, Object>();
				typeLanguageMap.put("@language",
						new LinkedHashMap<String, Object>());
				typeLanguageMap.put("@type",
						new LinkedHashMap<String, Object>());
	...
	private String selectTerm(String iri, List<String> containers,
			String typeLanguage, List<String> preferredValues) {
		final Map<String, Object> inv = getInverse();
		// 1)
		final Map<String, Object> containerMap = (Map<String, Object>) inv
				.get(iri);
		// 2)
	...
	public String getContainer(String property) {
		...
		final Map<String, Object> td = (Map<String, Object>) termDefinitions
				.get(property);
		if (td == null) {
			return null;
		}
	...
	public Object expandValue(String activeProperty, Object value)
			throws JsonLdError {
		...
		if (td != null && "@id".equals(td.get("@type"))) {
			...
			// @id
			rval.put("@id",
					expandIri(value.toString(), true, false, null, null));
			return rval;
		}
		// 2)
	...
	public Object getContextValue(String activeProperty, String string)
			throws JsonLdError {
	...
	public Map<String, Object> serialize() {
		...
		if (this.get("@base") != null
				&& !this.get("@base").equals(options.getBase())) {
		...
		for (final String term : termDefinitions.keySet()) {
			...
			if (definition.get("@language") == null
					...
					&& (definition.get("@reverse") == null || Boolean.FALSE
							.equals(definition.get("@reverse")))) {
				final String cid = this.compactIri((String) definition
						.get("@id"));
				...
				final String cid = this.compactIri((String) definition
						.get("@id"));
				final Boolean reverseProperty = Boolean.TRUE.equals(definition
						.get("@reverse"));
				...
				if (typeMapping != null) {
					defn.put("@type",
							JsonLdUtils.isKeyword(typeMapping) ? typeMapping
				...
				if (definition.get("@language") != null) {
					defn.put("@language", Boolean.FALSE.equals(lang) ? null
							: lang);
				}
				ctx.put(term, defn);
			}
