...
public interface RDFParser {
	 ...
	 * }
	 * 
	 * GRAPH: Must be the graph name/IRI. if no graph is present for a triple, add it to the "@default" graph
	 * TRIPLE: Must be a map with the following structure:
	 * {
	 * 		"subject" : SUBJECT
	 * 		"predicate" : PREDICATE
	 * 		"object" : OBJECT
	 * }
	 * 
	 * Each of the values in the triple map must also be a map with the following key-value pairs:
	 ...
	 *   
	 * The RDFDatasetUtils class has the following helper methods to make generating this format easier:
	 *  result = getInitialRDFDatasetResult();
	 *  triple = generateTriple(s,p,o);
