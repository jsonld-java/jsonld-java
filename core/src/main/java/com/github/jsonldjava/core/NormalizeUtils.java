...
class NormalizeUtils {
	...
	private Map<String, Object> bnodes;
	private List<Object> quads;
	private Options options;

	public NormalizeUtils(List<Object> quads, Map<String, Object> bnodes, UniqueNamer namer, Options options) {
					...
					for (int pgi = 0;;pgi++) {
						if (pgi == hashes.size()) {
							...
							for (int cai = 0; cai < quads.size(); ++cai) {
								Map<String,Object> quad = (Map<String, Object>) quads.get(cai);
								for (String attr: new String[] { "subject", "object", "name" }) {
									if (quad.containsKey(attr)) {
										Map<String,Object> qa = (Map<String, Object>) quad.get(attr);
						...
						List<String> group = duplicates.get(hashes.get(pgi));
						List<HashResult> results = new ArrayList<HashResult>();
						for (int n = 0;;n++) {
							if (n == group.size()) {
	...
	private static HashResult hashPaths(String id,
			Map<String, Object> bnodes, UniqueNamer namer,
			UniqueNamer pathNamer) {
		try {
			// create SHA-1 digest
			...
			List<Object> quads = (List<Object>) ((Map<String, Object>) bnodes.get(id)).get("quads");
			...
			for (int hpi = 0;; hpi++) {
				if (hpi == quads.size()) {
					...
					for (int hgi = 0;; hgi++) {
						...
						while (true) {
							...
							// build adjacent path
							String path = "";
							List<String> recurse = new ArrayList<String>();
				...
				// get adjacent bnode
				Map<String,Object> quad = (Map<String, Object>) quads.get(hpi);
				String bnode = getAdjacentBlankNodeName((Map<String, Object>) quad.get("subject"), id);
				...
				if (bnode != null) {
					...
					//String toHash = direction + (String) ((Map<String, Object>) quad.get("predicate")).get("value") + name;
					md1.update(direction.getBytes("UTF-8"));
					md1.update(((String)((Map<String, Object>) quad.get("predicate")).get("value")).getBytes("UTF-8"));
	...
	private static class Permutator {
		...
		public List<String> next() {
			List<String> rval = (List<String>)JSONLDUtils.clone(this.list);
