...
public class SesameTripleCallback implements JsonLdTripleCallback {
	...
	private void triple(String s, String p, String o, String graph) {
		...
		// object
		if (graph == null) {
			result = vf.createStatement(createResource(s), vf.createURI(p),
					createResource(o));
		} else {
			result = vf.createStatement(createResource(s), vf.createURI(p),
					createResource(o), createResource(graph));
		}
	...
	private void triple(String s, String p, String value, String datatype,
		...
		final URI datatypeURI = datatype == null ? null : vf
				.createURI(datatype);

		Value object;
		try {
			object = RDFParserHelper.createLiteral(value, language,
					datatypeURI, getParserConfig(), getParserErrorListener(),
					getValueFactory());
	...
	public Object call(final RDFDataset dataset) {
		if (handler != null) {
			try {
				handler.startRDF();
				for (final Entry<String, String> nextNamespace : dataset
		...
		for (String graphName : dataset.keySet()) {
			...
			for (final RDFDataset.Quad quad : quads) {
				if (quad.getObject().isLiteral()) {
					triple(quad.getSubject().getValue(), quad.getPredicate()
							.getValue(), quad.getObject().getValue(), quad
							.getObject().getDatatype(), quad.getObject()
							.getLanguage(), graphName);
				} else {
					triple(quad.getSubject().getValue(), quad.getPredicate()
							.getValue(), quad.getObject().getValue(), graphName);
				}
			}
		}
		if (handler != null) {
