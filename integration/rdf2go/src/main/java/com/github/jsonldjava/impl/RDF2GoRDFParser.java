...
public class RDF2GoRDFParser implements RDFParser {
	...
	private void handleStatement(RDFDataset result, Statement statement, URI context) {
		...
		if (object instanceof DatatypeLiteral) {
			...
			addStatement(result, context, subject, predicate, literal.getValue(), literal.getDatatype());
		} else if (object instanceof LanguageTagLiteral) {
			LanguageTagLiteral literal = (LanguageTagLiteral) object;
			addStatement(result, context, subject, predicate, literal.getValue(), literal.getLanguageTag());
	...
	public RDFDataset parse(Object input) throws JSONLDProcessingError {
		...
		if (input instanceof ModelSet) {
			...
			throw new JSONLDProcessingError("RDF2Go parser expects a Model or ModelSet object as input");
		}
